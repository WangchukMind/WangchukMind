...
type Conversation = { id: string; name: string; updatedAt: number; userId: string };
const conversations = new Map<string, Conversation[]>();

function getList(userId: string): Conversation[] {
...
conversationsRouter.post('/v1/conversations', requireAuth, (req, res) => {
  ...
  const item: Conversation = { id: randomUUID(), name, updatedAt: Date.now(), userId };
  const list = getList(userId);
  list.push(item);
  res.json({ ok: true, item });
});
